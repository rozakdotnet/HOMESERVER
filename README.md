...
<p>Server monitor designed for HomeServer and HomeLab where include Immich and AdguardHome api's, however it still works on vps. The temperature information only allowed on deicated server, storage information access allowed on dedicated server and KVM vps.</p>
...
<p>Block direct access to the configuration file (conf.ini).</p>
<p>For Apache:</p>
<pre> &lt;FilesMatch "\.ini$"&gt; 
  Require all denied 
  &lt;/FilesMatch&gt;</pre>
